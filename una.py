...
def target_build(staging_dir: Path, target_dir: Path, arch="x32"):
    colors.info(f"libmnl: target_build")
    repo_root = Path(__file__).parent
...
def target_install(staging_dir: Path, target_dir: Path, arch="x32"):
    colors.info(f"libmnl: target_install")
    repo_root = Path(__file__).parent
    
    # Install to staging for use by libnftnl and nftables
    colors.info(f"libmnl: installing to staging {staging_dir}")
    ...
    colors.info(f"libmnl: installing to target {target_dir}")
    ...
    colors.info(f"libmnl: pruning development files and documentation from target...")
    ...
    # Remove static libs from target to save space
